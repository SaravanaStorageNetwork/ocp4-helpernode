...
Download the Kickstart file for either [EL 7](examples/helper-ks-aarch64.cfg) or [EL 8](docs/examples/helper-ks8-aarch64.cfg) for the helper node.

__EL 7__
```
wget https://raw.githubusercontent.com/RedHatOfficial/ocp4-helpernode/master/docs/examples/helper-ks-aarch64.cfg -O helper-ks.cfg
```

__EL 8__
```
wget https://raw.githubusercontent.com/RedHatOfficial/ocp4-helpernode/master/docs/examples/helper-ks8-aarch64.cfg -O helper-ks.cfg
```
...
--boot hd,menu=on --location /var/lib/libvirt/ISO/CentOS-7-aarch64-Minimal-2003.iso 
...
--boot hd,menu=on --location /var/lib/libvirt/ISO/CentOS-8.1.1911-aarch64-dvd1.iso
...
> **NOTE** If using RHEL 7 - you need to enable the `rhel-7-server-rpms` and the `rhel-7-server-extras-rpms` repos. If you're using RHEL 8 you will need to enable `rhel-8-for-aarch64-baseos-rpms`, `rhel-8-for-aarch64-appstream-rpms`, and `ansible-2.9-for-rhel-8-aarch64-rpms`
...
Edit the [vars.yaml](examples/vars-aarch64.yaml) file with the mac addresses of the "blank" VMs.

```
cp docs/examples/vars-aarch64.yaml vars.yaml
```
